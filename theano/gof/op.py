...
class CLinkerOp(CLinkerObject):
    ...
    def c_code(self, node, name, inputs, outputs, sub):
        ...
        """
        raise utils.MethodNotDefined(f'{self.__class__.__name__}.c_code')
    ...
    def c_code_cleanup(self, node, name, inputs, outputs, sub):
        ...
        """
        raise utils.MethodNotDefined(f'{self.__class__.__name__}.c_code_cleanup')
...
class PureOp(object):
    ...
    def _get_test_value(cls, v):
        ...
        if isinstance(v, graph.Constant):
            ...
            return ret
        detailed_err_msg = utils.get_variable_trace_string(v)
        raise AttributeError(f'{v} has no test value {detailed_err_msg}')
...
class Op(utils.object2, PureOp, CLinkerOp):
    ...
    def make_py_thunk(self, node, storage_map, compute_map, no_recycling,
        ...
        p = node.op.debug_perform if debug else node.op.perform
    ...
    def make_node(self, *inputs):
        ...
        if any(inp.type != it for inp, it in zip(inputs, self.itypes)):
            raise TypeError(
                f"We expected inputs of types '{str(self.itypes)}' but got types '{[inp.type for inp in inputs]}' "
            )
    ...
    for i, arg in enumerate(args):
        try:
            ...
            if hasattr(arg, 'name') and arg.name is not None:
                missing_test_message(f"Argument {str(i)}('{arg.name}') has no test value")
            else:
                missing_test_message(f"Argument {str(i)} has no test value")
            return []

    return rval if len(rval) == 1 else [tuple(rval)]
...
class OpenMPOp(Op):
    ...
    def c_compile_args(self):
        ...
        return ['-fopenmp'] if self.openmp else []
    ...
    def c_headers(self):
        ...
        """
        self.update_self_openmp()
        return ["omp.h"] if self.openmp else []
...
int main( int argc, const char* argv[] )
{
        ...
        """
        return GCC_compiler.try_compile_tmp(
            src_code=code,
...
def simple_meth(tag):
    def f(self):
        if tag in self.code_sections:
            ...
            raise utils.MethodNotDefined(f'c_{tag}', type(self), type(self).__name__)
...
class COp(Op):
    ...
    def load_c_code(self, func_files):
        ...
        for i, code in enumerate(self.func_codes):
            if self.backward_re.search(code):
                ...
                if split[0].strip() != '':
                    raise ValueError(
                        f'Stray code before first #section statement (in file {func_files[i]}): {split[0]}'
                    )
                ...
                while n < len(split):
                    if split[n] not in self.SECTIONS:
                        raise ValueError(f"Unknown section type (in file {func_files[i]}): {split[n]}")
                ...
                raise ValueError(f"No valid section marker was found in file {func_files[i]}")
        ...
        if hasattr(self, 'params_type') and isinstance(self.params_type, theano.gof.ParamsType):
            ...
            for i in range(wrapper.length):
                try:
                    ...
                    params.append(
                        (
                            f'DTYPE_PARAM_{wrapper.fields[i]}',
    ...
    def get_c_macros(self, node, name, check_input=None):
        ...
        if check_input:
            ...
            for i, v in enumerate(variables):
                ...
                macro_name = f"DTYPE_{vname}"
                macro_value = f"npy_{v.dtype}"
    ...
    def _lquote_macro(self, txt):
        spl = txt.split('\n')
        res = [l + ' \\' for l in spl[:-1]]
        res.append(spl[-1])
        return '\n'.join(res)

    def get_sub_macros(self, sub):
        define_macros = [f"#define FAIL {self._lquote_macro(sub['fail'])}"]
        undef_macros = ["#undef FAIL"]
        if 'params' in sub:
    ...
    def c_code(self, node, name, inp, out, sub):
        if self.func_name is not None:
            ...
            params = f", {sub['params']}" if 'params' in sub else ""
            # Generate the C code
            return """
                %(define_macros)s
                {
