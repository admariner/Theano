...
try:
    ...
    if version != actual_version:
        raise ImportError(
            f"Version check of the existing lazylinker compiled file. Looking for version {version}, but found {actual_version}. Extra debug information: force_compile={force_compile}, _need_reload={_need_reload}"
        )
except ImportError:
    get_lock()
    try:
